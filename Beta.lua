...
function __gsoMenu:_menuAatrox()
    self.champ = self.menu:MenuElement({name = "Aatrox", id = "gsoaatrox", type = MENU, leftIcon = self.Icons["aatrox"] })
...
function __gsoMenu:_menuAhri()
    self.champ = self.menu:MenuElement({name = "Ahri", id = "gsoahri", type = MENU, leftIcon = self.Icons["ahri"] })
...
function __gsoMenu:_menuAkali()
    self.champ = self.menu:MenuElement({name = "Akali", id = "gsoakali", type = MENU, leftIcon = self.Icons["akali"] })
...
function __gsoMenu:_menuAlistar()
    self.champ = self.menu:MenuElement({name = "Alistar", id = "gsoalistar", type = MENU, leftIcon = self.Icons["alistar"] })
...
function __gsoMenu:_menuAmumu()
    self.champ = self.menu:MenuElement({name = "Amumu", id = "gsoamumu", type = MENU, leftIcon = self.Icons["amumu"] })
...
function __gsoMenu:_menuAnivia()
    self.champ = self.menu:MenuElement({name = "Anivia", id = "gsoanivia", type = MENU, leftIcon = self.Icons["anivia"] })
...
function __gsoMenu:_menuAnnie()
    self.champ = self.menu:MenuElement({name = "Annie", id = "gsoannie", type = MENU, leftIcon = self.Icons["annie"] })
...
function __gsoMenu:_menuAshe()
    self.champ = self.menu:MenuElement({id = "gsoashe", name = "Ashe", type = MENU, leftIcon = self.Icons["ashe"] })
...
function __gsoOrb:_orb(unit)
    ...
    if unitValid and self.canAA and self.dActionsC == 0 and gsoAIO.Callbacks._canAttack(unit) then
        self:_attack(unit)
    ...
    elseif self.canMove then
        ...
        if self.dActionsC == 0 and gsoAIO.Callbacks._canMove(unit) and Game.Timer() > self.lMove + (gsoAIO.Menu.menu.orb.delays.humanizer:Value()*0.001) and GetTickCount() > gsoAIO.Items.lastBotrk + 100 then
...
function __gsoDraw:__init()
    ...
    elseif gsoAIO.Load.meJhin then
        self.drawRanges = { q = true, qrange = 550+120, w = true, wrange = 3000, e = true, erange = 750, r = true, rrange = 3500 }
...
function __gsoUtils:_tick()
    ...
    for i = 1, #self.delayedActions do
        local dAction = self.delayedActions[i]
        if dAction and Game.Timer() > dAction.endTime then
            dAction.func()
            self.delayedActions[i] = nil
        end
    end
end
...
function __gsoUtils:_closestPointToLineSegment(p, p1, p2)
    local px,pz = p.x, p.z
    ...
    if t < 0 then
        return { x = ax, z = az }, false
    elseif t > 1 then
        return { x = bx, z = bz }, false
    else
...
function __gsoUtils:_checkHeroesCollision(p1, p2, width, id)
    for i = 1, #gsoAIO.OB.enemyHeroes do
        local hero = gsoAIO.OB.enemyHeroes[i]
        if hero.networkID ~= id then
            local heroPos = hero.pos
            local point,onLineSegment = self:_closestPointToLineSegment(heroPos, p1, p2)
            local distanceToPoint = self:_getDistance(heroPos, point)
...
function __gsoUtils:_checkMinionsCollision(p1, p2, width, id)
    for i = 1, #gsoAIO.OB.enemyMinions do
        local minion = gsoAIO.OB.enemyMinions[i]
        if minion.networkID ~= id then
            local minionPos = minion.pos
            local point,onLineSegment = self:_closestPointToLineSegment(minionPos, p1, p2)
            local distanceToPoint = self:_getDistance(minionPos, point)
...
function __gsoUtils:_movePos(unit, time)
    ...
    if unitPath.hasMovePath == true then
        local uPos  = unit.pos
        local ePos  = unitPath.endPos
        local dist1 = self:_getDistance(ePos, uPos)
        local predPos = uPos:Extended(ePos, time * unit.ms)
        local dist2 = self:_getDistance(predPos, uPos)
        if dist2 > dist1 then
            return ePos
        else
            return predPos
...
function __gsoUtils:_getClosestEnemy(sourcePos, enemyList, maxDistance)
    ...
    for i = 1, #enemyList do
        ...
        local distance = self:_getDistance(sourcePos, hero.pos)
        if self:_valid(hero, false) and distance < maxDistance then
...
function __gsoUtils:_isUnitBB(pos, id)
    for i = 1, #gsoAIO.OB.enemyHeroes do
        local unit = gsoAIO.OB.enemyHeroes[i]
...
function __gsoUtils:_countEnemyHeroesInRange(sourcePos, range, bb)
    ...
    for i = 1, #gsoAIO.OB.enemyHeroes do
        ...
        if self:_getDistance(sourcePos, unit.pos) < range + extraRange then
            count = count + 1
        end
    end
    return count
...
function __gsoUtils:_checkTimers(sT)
    local gT = GetTickCount() - ( self.maxPing * 1000 )
    local sq = sT.q
    local sw = sT.w
    local se = sT.e
    local sr = sT.r
    local q = gT - gsoAIO.Champ.lastQ
    local qq = gT - gsoAIO.WndMsg.lastQ
    local w = gT - gsoAIO.Champ.lastW
    local ww = gT - gsoAIO.WndMsg.lastW
    local e = gT - gsoAIO.Champ.lastE
    local ee = gT - gsoAIO.WndMsg.lastE
    local r = gT - gsoAIO.Champ.lastR
    local rr = gT - gsoAIO.WndMsg.lastR
    --[[assert(type(gT) == "number", "this is not number")
    assert(type(sq) == "number", "this is not number")
    assert(type(sw) == "number", "this is not number")
    assert(type(se) == "number", "this is not number")
    assert(type(sr) == "number", "this is not number")
    assert(type(q) == "number", "this is not number")
    assert(type(qq) == "number", "this is not number")
    assert(type(w) == "number", "this is not number")
    assert(type(ww) == "number", "this is not number")
    assert(type(e) == "number", "this is not number")
    assert(type(ee) == "number", "this is not number")
    assert(type(r) == "number", "this is not number")
    assert(type(rr) == "number", "this is not number")]]
...
function __gsoUtils:_getCastPos(spellData, sourcePos, target)
    local tP = target.pos
    local sD = spellData
    ...
    if HitChance > 0 and self:_getDistance(tP, castpos) < 500 then
        local distanceToSource = self:_getDistance(sourcePos, castpos)
        if distanceToSource < 150 or distanceToSource > sD.range then
            return nil
        end
        if sD.hCol then
            local id = target.type == Obj_AI_Hero and target.networkID or 0
            if self:_checkHeroesCollision(sourcePos, castpos, sD.width, id) then
                return nil
            end
        end
        if sD.mCol then
            local id = target.type == Obj_AI_Minion and target.networkID or 0
            if self:_checkMinionsCollision(sourcePos, castpos, sD.width, id) then
                return nil
            end
        end
        return castpos
...
function __gsoUtils:_isReady(spell, sT)
    return gsoAIO.Orb.dActionsC == 0 and self:_checkTimers(sT) and Game.CanUseSpell(spell) == 0
end

function __gsoUtils:_isReadyFast(spell, sT)
    return self:_checkTimers(sT) and Game.CanUseSpell(spell) == 0
end

function __gsoUtils:_castSpell(hkSpell)
    Control.KeyDown(hkSpell)
    Control.KeyUp(hkSpell)
    return true
end

function __gsoUtils:_castSpellTarget(hkSpell, range, sourcePos, target)
    local castpos = target and target.pos or nil
    local canCast = castpos and sourcePos and self:_getDistance(castpos, sourcePos) < range
          canCast = canCast and not self:_isUnitBB(castpos, target.networkID) and castpos:ToScreen().onScreen
    if canCast then
        local cPos = cursorPos
...
function __gsoUtils:_castSpellSkillShot(hkSpell, spellData, sourcePos, target)
    local castpos = target.x and target or self:_getCastPos(spellData, sourcePos, target)
    if castpos then
...
function __gsoAatrox:_canMove(target)
    if gsoAIO.Utils:_checkTimers({ q = 200, w = 200, e = 200, r = 200 }) then
        return true
    end
    return false
end
...
function __gsoAatrox:_canAttack(target)
    if gsoAIO.Utils:_checkTimers({ q = 300, w = 300, e = 300, r = 300 }) then
        return true
    end
    return false
end
...
function __gsoAshe:__init()
    ...
    self.oldWindUp = myHero.attackData.windUpTime
    self.wData = { delay = 0.25, range = 1200, width = 50, speed = 2000, sType = "line", col = true, hCol = true, mCol = true, out = true }
    self.rData = { delay = 0.25, range = 1500, width = 125, speed = 1600, sType = "line", col = false, hCol = true, mCol = false, out = true }
    gsoAIO.Callbacks:_setOnMove(function(target) self:_onMove(target) end)
...
function __gsoAshe:_onMove(target)
    ...
    -- USE R :
    local canR = ( isCombo and gsoAIO.Menu.champ.rset.combo:Value() ) or ( isHarass and gsoAIO.Menu.champ.rset.harass:Value() )
          canR = canR and (not isTarget or (isTarget and self.canR)) and gsoAIO.Utils:_isReady(_R, { q = 0, w = 250, e = 250, r = 1000 })
    if canR then
        if gsoAIO.Menu.champ.rset.rcd:Value() then
            local t = gsoAIO.Utils:_getClosestEnemy(mePos, gsoAIO.OB.enemyHeroes, gsoAIO.Menu.champ.rset.rdist:Value())
            if t and gsoAIO.Utils:_castSpellSkillShot(HK_R, self.rData, mePos, t) then
        ...
        if gsoAIO.Menu.champ.rset.rci:Value() then
            local t = gsoAIO.Utils:_getImmobileEnemy(mePos, gsoAIO.OB.enemyHeroes, 1000)
            if t and gsoAIO.Utils:_castSpellSkillShot(HK_R, {}, mePos, t.pos) then
                self.lastR = GetTickCount()
                self.canW = false
                --print("r")
                return true
    ...
    if canQ and isTarget and gsoAIO.Utils:_isReadyFast(_Q, { q = 1000, w = 250, e = 250, r = 250 }) then
        local aaSpeed = ( myHero.attackSpeed + 0.15 + (myHero:GetSpellData(_Q).level * 0.05) ) * gsoAIO.Orb.baseAASpeed
        local numAS   = aaSpeed >= 2.5 and 2.5 or aaSpeed
        if Game.Timer() > gsoAIO.Orb.lAttack + ( 1 / numAS ) - 0.15 - gsoAIO.Utils.maxPing and gsoAIO.Utils:_castSpell(HK_Q) then
            self.oldWindUp = gsoAIO.Orb.windUpT
            self.asNoQ = myHero.attackSpeed
    ...
    if not isTarget or afterAttack then
        ...
        local canW = ( isCombo and gsoAIO.Menu.champ.wset.combo:Value() ) or ( isHarass and gsoAIO.Menu.champ.wset.harass:Value() )
              canW = canW and (not isTarget or (isTarget and self.canW)) and gsoAIO.Utils:_isReady(_W, { q = 0, w = 1000, e = 250, r = 250 })
        if canW then
            local t = isTarget and target or gsoAIO.TS:_getTarget(1200, false, false, gsoAIO.OB.enemyHeroes)
            --assert(t and t.type == Obj_AI_Hero, "nohero")
...
function __gsoAshe:_aaSpeed()
    local num = Game.Timer() - self.QEndTime + gsoAIO.Utils.maxPing
    if num > -self.oldWindUp and num < 2 then
        return self.asNoQ
    end
...
function __gsoAshe:_canMove(target)
    if gsoAIO.Utils:_checkTimers({ q = 0, w = 200, e = 200, r = 200 }) then
        return true
    end
    return false
end
...
function __gsoAshe:_canAttack(target)
    if gsoAIO.Utils:_checkTimers({ q = 0, w = 300, e = 300, r = 300 }) then
        return true
    end
    return false
end
...
function __gsoAshe:_tick()
    ...
    -- semi r
    if gsoAIO.Menu.champ.rset.semirashe.enabled:Value() and gsoAIO.Utils:_isReady(_R, { q = 0, w = 250, e = 250, r = 1000 }) then
        local rTargets = {}
        for i = 1, #gsoAIO.OB.enemyHeroes do
            local hero = gsoAIO.OB.enemyHeroes[i]
            local heroName = hero.charName
            local isFromList = gsoAIO.Menu.champ.rset.semirashe.useon[heroName] and gsoAIO.Menu.champ.rset.semirashe.useon[heroName]:Value()
            local hpPercent = gsoAIO.Menu.champ.rset.semirashe.semilow:Value() and gsoAIO.Menu.champ.rset.semirashe.semip:Value() or 100
            local isLowHP = isFromList and ( ( hero.health + ( hero.hpRegen * 3 ) ) * 100 ) / hero.maxHealth <= hpPercent
            if isLowHP then
                rTargets[#rTargets+1] = hero
            end
        end
        local rrange = gsoAIO.Menu.champ.rset.semirashe.rrange:Value()
        local rTarget = gsoAIO.TS:_getTarget(rrange, false, false, rTargets)
        if rTarget and gsoAIO.Utils:_castSpellSkillShot(HK_R, self.rData, myHero.pos, rTarget) then
            self.lastR = GetTickCount()
        end
    end
end
...
function __gsoDraven:_onAttack(target)
    local isTarget = target and target.type == Obj_AI_Hero
    local canQ = ( gsoAIO.Menu.menu.orb.keys.combo:Value() and gsoAIO.Menu.champ.qset.combo:Value() ) or ( gsoAIO.Menu.menu.orb.keys.harass:Value() and gsoAIO.Menu.champ.qset.harass:Value() )
    if canQ and isTarget and gsoAIO.Utils:_castSpell({ q = 1000, w = 0, e = 250, r = 250 }, _Q, HK_Q) then
        self.lastQ = GetTickCount()
    end
end
...
function __gsoDraven:_canMove(target)
    if gsoAIO.Utils:_checkTimers({ q = 0, w = 0, e = 250, r = 250 }) then
        return true
    end
    return false
end
...
function __gsoDraven:_canAttack(target)
    if gsoAIO.Utils:_checkTimers({ q = 0, w = 0, e = 350, r = 350 }) then
        return true
    end
    return false
end
...
function __gsoDraven:_tick()
    ...
    -- semi r
    if gsoAIO.Menu.menu.gsodraven.rset.semirdraven.enabled:Value() and gsoAIO.Utils:_isReady({ q = 0, w = 100, e = 250, r = 1000 }, _R) then
        local rTargets = {}
        for i = 1, #gsoAIO.OB.enemyHeroes do
            local hero = gsoAIO.OB.enemyHeroes[i]
            local heroName = hero.charName
...
function __gsoEzreal:_canMove(target)
    if gsoAIO.Utils:_checkTimers({ q = 175, w = 175, e = 250, r = 1000 }) then
        return true
    end
    return false
end
...
function __gsoEzreal:_canAttack(target)
    if gsoAIO.Utils:_checkTimers({ q = 350, w = 350, e = 350, r = 1100 }) then
        return true
    end
    return false
end
...
function __gsoEzreal:_tick()
    ...
    -- semi r
    if gsoAIO.Menu.menu.gsoezreal.rset.semirez.enabled:Value() and gsoAIO.Utils:_isReady({ q = 350, w = 350, e = 550, r = 1100 }, _R) then
        local rTargets = {}
        for i = 1, #gsoAIO.OB.enemyHeroes do
            local hero = gsoAIO.OB.enemyHeroes[i]
            local heroName = hero.charName
    ...
    local canQ = gsoAIO.Menu.menu.gsoezreal.autoq.enable:Value() and not gsoAIO.Menu.menu.orb.keys.combo:Value() and not gsoAIO.Menu.menu.orb.keys.harass:Value()
          canQ = canQ and gsoAIO.Utils:_isReady({ q = 1000, w = 350, e = 550, r = 1100 }, _Q)
...
function __gsoJhin:_onMove(target)
    ...
    local mePos = myHero.pos
    if not isTarget or afterAttack then
        local canE = ( isCombo and gsoAIO.Menu.menu.gsojhin.eset.combo:Value() ) or ( isHarass and gsoAIO.Menu.menu.gsojhin.eset.harass:Value() )
        if canE and gsoAIO.Utils:_isReady(gT, { q = 350, w = 850, e = 1000, r = 350 }, _E) and (not isTarget or (isTarget and self.canE)) then
        ...
        local canW = ( isCombo and gsoAIO.Menu.menu.gsojhin.wset.combo:Value() ) or ( isHarass and gsoAIO.Menu.menu.gsojhin.wset.harass:Value() )
        if canW and gsoAIO.Utils:_isReady(gT, { q = 350, w = 1500, e = 350, r = 350 }, _W) and (not isTarget or (isTarget and self.canW)) then
            local wTarget = isTarget and target or gsoAIO.TS:_getTarget(1350, false, false, gsoAIO.OB.enemyHeroes)
            if wTarget and gsoAIO.Utils:_castSpellSkillshot(mePos, wTarget, { delay = 0.25, range = 1250, width = 75, speed = 1600, sType = "line", col = false }, HK_W) then
                self.canQ = false
                return true
        ...
        local canQ = (isCombo and gsoAIO.Menu.menu.gsojhin.qset.combo:Value()) or (isHarass and gsoAIO.Menu.menu.gsojhin.qset.harass:Value())
        if canQ and (not isTarget or (isTarget and self.canQ)) then
            if isTarget and not gsoAIO.Utils:_isUnitBB(target.pos, target.networkID) and gsoAIO.Utils:_castSpellTarget(target.pos, _Q, HK_Q, gT, { q = 1000, w = 350, e = 350, r = 3350 }) then
                self.lastQ = GetTickCount()
                return true
            end
        end
...
function __gsoJinx:_canMove(target)
    if gsoAIO.Utils:_checkTimers({ q = 0, w = 500, e = 0, r = 500 }) then
        return true
    end
    return false
end
...
function __gsoJinx:_canAttack(target)
    if gsoAIO.Utils:_checkTimers({ q = 0, w = 600, e = 0, r = 600 }) then
        return true
    end
    return false
end
...
function __gsoJinx:_tick()
    ...
    -- semi r
    if gsoAIO.Menu.menu.gsojinx.rset.semirjinx.enabled:Value() and gsoAIO.Utils:_isReady({ q = 0, w = 550, e = 75, r = 1000 }, _R) then
        local rTargets = {}
        for i = 1, #gsoAIO.OB.enemyHeroes do
            local hero = gsoAIO.OB.enemyHeroes[i]
            local heroName = hero.charName
...
function __gsoKogMaw:_canMove(target)
    if gsoAIO.Utils:_checkTimers({ q = 250, w = 0, e = 250, r = 250 }) then
        return true
    end
    return false
end
...
function __gsoKogMaw:_canAttack(target)
    ...
    local canW = ( gsoAIO.Menu.menu.orb.keys.combo:Value() and gsoAIO.Menu.menu.gsokog.wset.combo:Value() ) or ( gsoAIO.Menu.menu.orb.keys.harass:Value() and gsoAIO.Menu.menu.gsokog.wset.harass:Value() )
          canW = canW and gsoAIO.Utils:_isReadyFast({ q = 250, w = 1000, e = 250, r = 250 }, _W)
    ...
    if gsoAIO.Utils:_checkTimers({ q = 350, w = 0, e = 350, r = 350 }) then
        return true
    end
    return false
end
...
function __gsoKogMaw:_tick()
    ...
    local canR = gsoAIO.Utils:_isReady({ q = 350, w = 150, e = 350, r = 750 }, _R)
...
function __gsoSivir:_canMove(target)
   if gsoAIO.Utils:_checkTimers({ q = 250, w = 0, e = 0, r = 0 }) then
        return true
    end
    return false
end
...
function __gsoSivir:_canAttack(target)
   if gsoAIO.Utils:_checkTimers({ q = 350, w = 0, e = 0, r = 0 }) then
        return true
    end
    return false
end
...
function __gsoTeemo:_canMove(target)
   if gsoAIO.Utils:_checkTimers({ q = 250, w = 0, e = 0, r = 250 }) then
        return true
    end
    return false
end
...
function __gsoTeemo:_canAttack(target)
   if gsoAIO.Utils:_checkTimers({ q = 350, w = 0, e = 0, r = 350 }) then
        return true
    end
    return false
end
...
function __gsoTristana:_onAttack(target)
    ...
    if isTarget and canQ and gsoAIO.Utils:_castSpell({ q = 1000, w = 750, e = 0, r = 0 }, _Q, HK_Q) then
        self.asNoQ = myHero.attackSpeed
        self.lastQ = GetTickCount()
    end
end
...
function __gsoTristana:_canMove(target)
   if gsoAIO.Utils:_checkTimers({ q = 0, w = 500, e = 100, r = 100 }) then
        return true
    end
    return false
end
...
function __gsoTristana:_canAttack(target)
    ...
    if isTarget then
        ...
        if canR then
            ...
            for i = 1, #gsoAIO.OB.enemyHeroes do
                ...
                local canRonUnit = unitPos and gsoAIO.Utils:_valid(unit, false) and gsoAIO.Utils:_getDistance(unitPos, mePos) < meRange + unit.boundingRadius
                      canRonUnit = canRonUnit and not gsoAIO.Utils:_isUnitBB(unitPos, unit.networkID)
                if canRonUnit then
        ...
        if canE then
            ...
            if not gsoAIO.Utils:_isUnitBB(targetPos, targetID) and gsoAIO.Utils:_castSpellTarget(targetPos, _E, HK_E, gT, { q = 0, w = 350, e = 1000, r = 250 }) then
                self.lastE = GetTickCount()
...
function __gsoVayne:_onMove(target)
    ...
    if isTarget and afterAttack then
        ...
        local canE = targetPos and ( ( isCombo and gsoAIO.Menu.menu.gsovayne.eset.combo:Value() ) or ( isHarass and gsoAIO.Menu.menu.gsovayne.eset.harass:Value() ) )
              canE = canE and not gsoAIO.Utils:_isUnitBB(targetPos, target.networkID)
        if canE then
...
function __gsoVayne:_canMove(target)
    if gsoAIO.Utils:_checkTimers({ q = 200, w = 0, e = 400, r = 0 }) then
        return true
    end
    return false
end
...
function __gsoVayne:_canAttack(target)
    if gsoAIO.Utils:_checkTimers({ q = 300, w = 0, e = 500, r = 0 }) then
        return true
    end
    return false
end
